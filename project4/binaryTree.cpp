...
static string findPathHelperstr(Node *p, const string &s){
    ...
    if(findPathHelperbool(p->leftSubtree(),s)) return "0"+findPathHelperstr(p->leftSubtree(),s);
    if(findPathHelperbool(p->rightSubtree(),s)) return "1"+findPathHelperstr(p->rightSubtree(),s);
    return "-1";
}
...
static bool covered_byHelper(Node *self, Node *other){
    ...
    covered_byHelper(self->leftSubtree(), other->leftSubtree())
